...
class DB:
    ...
    def __init__(self, location: str) -> None:
        ...
        """)
        cur.execute(
            "CREATE TABLE IF NOT EXISTS TEACHER (id TEXT, name TEXT, PRIMARY KEY (id, name))"
        )
        cur.execute(
        ...
        cur.execute(
            "CREATE TABLE IF NOT EXISTS RESULT (courseId TEXT, yearsem TEXT, name TEXT, teacher TEXT, time TEXT, studentLimit INTEGER, studentCount INTEGER, lastEnroll INTEGER, PRIMARY KEY (courseId))"
        )

    def add_rate(
        self, rowId: str, courseId: str, teacherId: str, content: str, contentEn: str
    ):
        cur = self.con.cursor()
        cur.execute(
            "INSERT OR REPLACE INTO RATE (rowId, courseId, teacherId, content, contentEn) VALUES (?, ?, ?, ?, ?)",
            (rowId, courseId, teacherId, content, contentEn),
        )
        self.con.commit()
    ...
    def add_teacher(self, id: str, name: str):
        cur = self.con.cursor()
        cur.execute(
            "INSERT OR REPLACE INTO TEACHER (id, name) VALUES (?, ?)", (id, name)
        )
        self.con.commit()
    ...
    def add_result(
        ...
        cur = self.con.cursor()
        cur.execute(
            "INSERT OR REPLACE INTO RESULT (courseId, yearsem, name, teacher, time, studentLimit, studentCount, lastEnroll) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (
                yearsem + courseId,
                yearsem,
                name,
    ...
    def get_course(self, y: str, s: str):
        cur = self.con.cursor()
        request = cur.execute("SELECT teaNam FROM COURSE WHERE y = ? AND s = ?", [y, s])
        response = request.fetchall()
    ...
    def get_this_semester_course(self, y: str, s: str):
        cur = self.con.cursor()
        request = cur.execute(
            "SELECT DISTINCT subNum FROM COURSE WHERE y = ? AND s = ?", [y, s]
        )
        response = request.fetchall()
    ...
    def is_course_exist(self, courseId: str, dp: dict):
        cur = self.con.cursor()
        request = cur.execute(
            "SELECT COUNT(*) FROM COURSE WHERE id = ? AND dp1 = ? AND dp2 = ? AND dp3 = ?",
            [courseId, dp["dp1"], dp["dp2"], dp["dp3"]],
        )
    ...
    def is_rate_exist(self, courseId: str):
        cur = self.con.cursor()
        request = cur.execute(
            "SELECT COUNT(DISTINCT courseId) FROM RATE WHERE courseId = ?", [courseId]
        )
        response = request.fetchone()
        return response[0] > 0
